...
export default function CircularGallery({
  ...
  textColor = "#ffffff",
  borderRadius = 0.05,
  font = "bold 60px var(--font-noble-display), sans-serif",
  scrollSpeed = 2,
  scrollEase = 0.05,
  ...
  return (
    <div className="w-full h-full overflow-hidden relative">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="flex justify-center items-end h-full gap-8">
          {galleryItems.map((item, idx) => (
            <Tooltip key={item.text + idx}>
              <TooltipTrigger asChild>
                <Link href={item.link || "#"} legacyBehavior passHref>
                  <a
                    className="block w-[320px] h-[320px] rounded-2xl pointer-events-auto group transition-all duration-300 bg-black/10 hover:bg-black/60"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ position: "relative" }}
                  >
                    {/* Canvas is rendered below, so this overlay is clickable */}
                  </a>
                </Link>
              </TooltipTrigger>
              <TooltipContent sideOffset={8}>{item.text}</TooltipContent>
            </Tooltip>
          ))}
        </div>
      </div>
